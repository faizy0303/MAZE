// FORWARD REPEATED A*
//UI implemented code. Breaking ties done with greater g values. Final path states stored
...
class Binaryheap1 
{
	...
	void heapifyup(int index)
	{
		...
		if(tempint.f==heap[parentt(index)].f)//added loop zhilmil tie with lesser g values
		{
	...
	void heapifydown(int index)
	{
		...
		if (r < heapsize && (heap[r].f < heap[min].f))
			min = r;
		if(r < heapsize && heap[r].f == heap[min].f)  //added loop zhilmil tie with greater g values
		{
			if(heap[r].g > heap[min].g)
				min = r;
		}
		if(l < heapsize && heap[l].f == heap[min].f)  //added loop zhilmil tie with greater g values
		{
		...
		x[mazearr[z].r][mazearr[z].c] = s_start;		//random maze  //anwar swapped
			...
			while((s_start.x!=goal_x) || (s_start.y!=goal_y) )
			{
				...
				if(open.isnull())
				{
					...
					System.out.println("Time taken by forward2 is: "+time);
