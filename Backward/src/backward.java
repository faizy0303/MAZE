// BACKWARD Repeated A*
//UI implemented code. Breaking ties done. Final path states stored
...
// Reading a maze from textfile
		...
		x[mazearr[z].r][mazearr[z].c] = s_start;		//random maze  //anwar swapped
			...
			while((s_start.x!=goal_x) || (s_start.y!=goal_y) )
			{
				...
				ComputePath(open,s_start,closed,x,s_start.x,s_start.y);		//backward (open,s_goal,closed,x,goal_x,goal_y);
				...
				if((goal_x!=temp.x)||(goal_y!=temp.y))  //backward (s_start.x!=temp.x)||(s_start.y!=temp.y)
				{
					for (int s=0; s<i; s++)
					{
